...
<nav aria-label="Breadcrumb" class="py-2 {customClass}">
  <div class="{classes.nav} rounded-full px-4 py-2 inline-flex shadow-sm">
    <ol
      ...
      {#each displayItems as item, index}
        ...
        >
          {#if index > 0}
            ...
            <Separator class="w-4 h-4 {classes.separator}" />
          {/if}
          ...
          {:else if item.href}
            ...
            <a
              href={item.href}
              class="{classes.link} transition-colors duration-200 text-sm font-medium flex items-center gap-1.5"
              itemprop="item"
            >
              {#if showHome && index === 0}
                <Home class="w-4 h-4 {classes.icon}" />
              {/if}
              {#if item.icon && index > 0}
                {@const Icon = item.icon}
                <Icon class="w-4 h-4 {classes.icon}" />
              {/if}
          ...
          {:else}
            <!-- Active/current item -->
            <span
              class="{classes.active} text-sm flex items-center gap-1.5"
              aria-current="page"
              itemprop="item"
            >
              {#if showHome && index === 0}
